...
ARGUMENTS = [
    DeclareLaunchArgument('world_name', default_value='world2.sdf', description='Name of the world to load. Match with map if using Nav2.'),
    ...
    DeclareLaunchArgument('entity', default_value='handia', description='Name of the robot'),
    DeclareLaunchArgument('robot_description_topic', default_value='robot_description', description='Robot description topic.'),
]

def get_robot_description():
    pkg_handia_gazebo = get_package_share_directory('roshandia_pkg')
    robot_description_path = os.path.join(pkg_handia_gazebo, 'urdf', 'hand.urdf')
    mappings = {}
    ...
    robot_desc = robot_desc.replace(
        'package://roshandia_pkg/', f'file://{pkg_handia_gazebo}/'
    )
    return robot_desc
...
def generate_launch_description():
    ...
    world_path = PathJoinSubstitution([pkg_handia_gazebo,'worlds',world_name])
    bridge_config_file_path = os.path.join(pkg_handia_gazebo, 'config', 'bridge_config.yaml')
    ...
    ld.add_action(
        Node(
            package='roshandia_pkg',
            executable='joinStatePublisherSim',
            output = 'screen',
        )
    )
    return ld